...
const STORAGE_KEY = "wafer.metal.v1";
...
function readInitial(preset: MetalPresetId): MetalParams {
  ...
  try {
    ...
    // Corrupt or unavailable storage must never cost the user the material.
    return fallback;
  }
}
 ...
 * clamped to the schema first, so a stale entry can never brick a surface.
 */
export function useMetal(initialPreset: MetalPresetId = "alloy"): UseMetalResult {
