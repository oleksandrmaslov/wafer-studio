 ...
 *
 * ARCHIVED. This is the earlier "one shared light" dispersion material, kept
 * self-contained so it can be lifted into another project wholesale. It is no
 * longer wired into Wafer Studio, which now uses the chromatic metal in
 * `../shader/`.
 *
 * To reuse: this folder plus `DispersionField.tsx` and the `--light-x` /
 * `--light-y` contract is everything it needs.
 *
...
/**
 * One material at three amplitudes: `scale`, `detail`, and `warp` barely move
 * between them while `dispersion` and `specular` do.
 */
export const ABERRATION_PRESETS = {
  ...
  /** Tight, bright, heavily dispersed, nearly still. */
  mark: {
    scale: 1.1,
    detail: 2.3,
    warp: 1,
